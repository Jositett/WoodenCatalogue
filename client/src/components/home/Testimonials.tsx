...
const Testimonials: React.FC<TestimonialsProps> = ({ testimonials, isLoading }) => {
  const renderStars = (rating: number) => {
    return Array.from({ length: 5 }).map((_, i) => (
      <Star 
        key={i} 
        className={`h-4 w-4 ${i < rating ? 'text-[#D4B483] fill-[#D4B483]' : 'text-gray-300'}`} 
      />
    ));
  };
  ...
  const renderSkeleton = () => (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      {[...Array(3)].map((_, index) => (
        <div key={index} className="bg-white rounded-sm p-8">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map((testimonial) => (
              <div 
                key={testimonial.id} 
                className="testimonial-card p-8 text-[#333333] bg-white rounded-sm shadow-md transition-all duration-300 hover:-translate-y-2 hover:shadow-lg"
              >

    ...
    if (products) {
      ...
      if (filters.features !== 'all') {
        filtered = filtered.filter(product => 
          product.features && product.features.some(feature => feature.toLowerCase().includes(filters.features.toLowerCase()))
        );
      }
      ...
      <div className="container mx-auto px-4">
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="bg-white dark:bg-gray-800 rounded-sm overflow-hidden shadow-md p-4">
                ...
                <Skeleton className="h-4 w-1/3 mb-4 dark:bg-gray-700" />
                <div className="flex justify-between items-center">
                  <Skeleton className="h-6 w-1/4 dark:bg-gray-700" />
                  <Skeleton className="h-10 w-1/3 dark:bg-gray-700" />
                </div>
              </div>
            ))}
          </div>
          ...
          <>
            ...
            {visibleProducts < filteredProducts.length && (
              <div className="text-center">
                <Button
                  onClick={loadMore}
                  className="bg-[#5C4033] hover:bg-[#7D5A50] dark:bg-amber-600 dark:hover:bg-amber-700 text-white font-medium px-8 py-6 rounded-sm transition duration-300"
                >
                  Load More Doors
                </Button>
              </div>
            ...
            {filteredProducts.length === 0 && (
              <div className="text-center py-12">
                <h3 className="text-2xl font-playfair text-[#5C4033] dark:text-amber-200 mb-4">No doors match your filters</h3>
                <p className="mb-6 dark:text-gray-300">Try adjusting your filter criteria to see more options.</p>
                <Button
                  onClick={() => setFilters({
                    style: 'all',
                    woodType: 'all',
                  ...
                  variant="outline"
                  className="border-[#7D5A50] text-[#7D5A50] hover:bg-[#7D5A50] hover:text-white dark:border-amber-600 dark:text-amber-600 dark:hover:bg-amber-600"
                >
                  Reset Filters
                </Button>
              </div>
